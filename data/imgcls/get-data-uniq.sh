...
function get_msl {
  ...
  for split in train test val; do
    ...
    tqdm --total "$(wc -l < ${table})" < $table |
      grep -v -E '\-(fv|fh|brt|r90|r180|r270).jpg' |
...
function get_hirise {
  ...
  echo "restructuring : $src -> $tgt"
  tqdm --total "$(wc -l < ${table})" < $table |
    grep -v -E '\-(fv|fh|brt|r90|r180|r270).jpg' |   # exclude augmentations (in train and val)
    while read img cls split; do
      label=${classes[$cls]}
      dst=$tgt/$split/$label/$img
      [[ -f $dst ]] || cp $images/$img $dst
    done
